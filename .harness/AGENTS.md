...
- 本项目所有测试、开发、构建、脚本运行前，必须先激活 Conda 环境 `LC`。
- 在 PowerShell 中执行项目命令时，使用同一个 shell 会话运行: `conda activate LC; <command>`。
- 不要使用 `base` 环境或系统 Python 直接运行后端、测试、脚本、格式化或类型检查命令。
- 如果发现当前环境不是 `LC`，先切换到 `LC`，再继续执行任务。
